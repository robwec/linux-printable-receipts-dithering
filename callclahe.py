...
def apply_CLAHE(myimage, cliplimit): #values of 0.x-2.0 seem alright. 0, negative, and big values look bad.
	...
	try:
		newplanezero = clahe.apply(lab_planes[0])
	except:
		print("clahe error")
		import pdb; pdb.set_trace()
	
	lab = cv2.merge((newplanezero,) + lab_planes[1:])
...
######
##main
def mainthing(imagename, colortype = "color", writeimage = True, scaletype = "noscale"):
	...
	elif scaletype == "receipt":
		myimage = scale_minSide_to384(myimage)
	else: #don't scale. Just dither a huge image for fun. Print it on a regular printer maybe.
		pass
	...
	if colortype == "graycucuu":
		...
		usingcheapoprinter = (scaletype != "fourbysixlabel")
...
if __name__ == "__main__":
	...
	mainthing(imagename, colortype, writeimage = True, scaletype = fourbysixlabel)
